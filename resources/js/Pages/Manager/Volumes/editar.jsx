...
const Page = () => {
    const { idioma, idiomas, volume } = usePage().props;

    const { data, setData, post, processing, errors } = useForm(volume);
    ...
    const handleSubmit = (e) => {
        ...
        post(route('Manager.Volumes.atualizar', {id: volume.id, lang: idioma_url}), {
            preserveScroll: true,
        });
        console.log(data);
    ...
    return (
        <AdminLayout>
            <Breadcrumb icon={faList} items={breadcrumbItems} current="Editar" idioma={idioma.codigo} id={volume.id} />
